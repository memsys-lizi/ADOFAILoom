    ...
    internal sealed class VisualEventMutationEngine
    {
            ...
            CancellationToken cancellationToken)
        {
            return dispatcher.InvokeAsync(
                () => Add(expectedRevision, inputs, create),
        ...
        {
            return dispatcher.InvokeAsync(
                () => Remove(expectedRevision, references, requiredType),
                McpProtocol.MainThreadTimeout,
                cancellationToken);
        }
            ...
            Func<scnEditor, TInput, LevelEvent> create)
        {
            ...
            VisualEventReference[] references = staged
                .Select((levelEvent, index) => VisualEventReader.CreateReference(
            ...
            where TInput : class
        {
            ...
            foreach (TInput input in inputs)
            {
                ...
                VisualEventReference reference = getReference(requiredInput) ??
            ...
            VisualEventReference[] references = staged
                .Select(update => VisualEventReader.CreateReference(
                    VisualEventCollection.LevelEvents,
                    update.Index,
                    update.Event))
                .ToArray();
